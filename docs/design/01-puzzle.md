...
- A pour moves the **entire top run of one color**, unless the target runs out of
  room first, in which case it moves as much as fits.
- The target must be **empty, or have the same color on top**.
- A bottle **full of one color is finished** and can no longer be poured from.
...
Four units of a color is the win condition, not an accident of the deal. Destroy
part of a stack and the rest of that color is stranded: it can never make four
...
- A color a blast took units from is **spilled**.
- A spilled color is finished when it is **gathered**, rather than when it
  reaches four.
...
The rule underneath was always **every bottle holds one color, and no color is
in two bottles**. Without spilling those two clauses collapse into "four of a
color in one bottle", since every deal makes exactly four of each and four
