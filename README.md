...
| `tools/verify-live.mjs` | the same question from the other end: exports, unused helpers, classes and ids |
| `tools/sound-lab/` | dev tool: plays every candidate sound side by side. Run `node tools/sound-lab/make.mjs` first |
| `tests/` | unit suite in `node`, browser suite in `tests/e2e/`. No dependencies in either |
...
its own sources and its own build id. It is deliberately left out of the app's
precache, so the two cannot cache over each other.
