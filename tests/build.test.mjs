...
describe('the generators', () => {
  /* A generated source is written by a template inside a tool, and the template
     ...
     reads. When the sources moved from globals to ES modules, two of the four
     templates were updated and two were not, so `npm run order` and `npm run
     pars` sat there writing `globalThis.ORDER = ...` into a build that imports
     it. Nothing failed until somebody regenerated, which is the worst possible
     moment to find out: mid-regeneration, with the old table already gone.

     Checked on the tools rather than on their output, because the output is
     only wrong after a regeneration and by then the damage is done. */
  const tools = readdirSync(join(root, 'tools')).filter(f => f.endsWith('.mjs'));
  it('write ES modules, not globals, into src', () => {
    const bad = [];
    for (const f of tools){
      const src = read(`tools/${f}`);
      if (!/writeFileSync\([^)]*src\//.test(src)) continue;      // writes nothing into src
      for (const m of src.matchAll(/globalThis\.([A-Za-z_$][\w$]*)\s*=/g))
        bad.push(`tools/${f} writes globalThis.${m[1]} into a source the build imports`);
    }
    equal(bad, [], 'a generator still emits a global instead of an export');
  });
  it('leave a first line naming the command that rewrites them', () => {
    /* The convention the repo relies on to stop anyone hand-editing one. */
    const dirs = ['src/js', ...gameDirs().map(g => `src/${g}/js`)];
    const generated = dirs.flatMap(d => pureOf(d).map(f => `${d}/${f}`))
      .filter(p => /^\/\*\s*Generated by/.test(read(p)));
    assert(generated.length >= 4, `found ${generated.length} generated sources, so this is not reading them`);
    const wrong = generated.filter(p => !/`npm run [\w:]+`/.test(read(p).split('\n')[0]));
    equal(wrong, [], 'a generated source does not name the command that writes it');
  });
});
