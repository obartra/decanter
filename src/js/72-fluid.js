...
const Fluid = (() => {
  ...
  function measure(){
    ...
    els.forEach((b, i) => {
      ...
      bottles[i] = {
        i,
        x0: g.left - box.left + 3,
        x1: g.right - box.left - 3,
        yTop: g.top - box.top + 2,
        yBot: g.bottom - box.top - 3,
        cap: g.height - 5
      };
    });
  ...
  function step(dt){
    ...
    if (bandT > 0.1){
      ...
      Object.keys(per).forEach(bi => {
        const b = bottles[bi]; if (!b) return;
        const counts = per[bi];
  ...
  function draw(){
    ...
    bottles.forEach(b => {
      if (!b) return;
      /* the glass, plus the air above it so a jet in flight is visible */
      ctx.rect(b.x0 - 1, 0, b.x1 - b.x0 + 2, b.yBot - 0);
    });
    ctx.clip();
  ...
  function pour(move){
    return new Promise(resolve => {
      ...
      jet = {
        ...
        y: dst.yTop - 26,
        vx: dir * 34, vy: 46,
        quota: Math.max(12, perUnit * move.n),
