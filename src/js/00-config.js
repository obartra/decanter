...
const CONFIG = {
  ...
  sectionSize: 10,
  /* How many boards in each chapter are the other game.

     Two per ten, but deliberately not every fifth one. At a fixed stride a
     player has the rule after one chapter and every bubble board afterwards is
     an appointment; scattered, it stays the thing it is for, which is the run
     turning into something else without warning. Which two is decided by
     arithmetic on the chapter number rather than by a list, so level 12 is the
     same board for everyone forever and there is nothing to keep in step with
     the par table.

     The graded range does not change and neither does the pour game's curve.
     ORDER is monotone in measured difficulty and a subsequence of a sorted
     sequence is still sorted, so it does not matter which boards are taken out:
     no par is recomputed and no ordering regenerated by turning this on. */
  bubblePerChapter: 2,
  /* A name and a tint per chapter. Sections past these are called Reserve N and
     hand nothing over, which is what half the game was: twelve chapters, six
     names, and every tool a player would ever hold by level 61. The Furnace is
     the seventh, and it exists because the blast needed somewhere to be given
     out from — a tool that appeared in the tool row with no chapter behind it
     would be the first one that ever had. */
