...
/* Which places in a chapter belong to the other game.
   ...
   Two per ten, and the two is here in the shape of the function rather than in a
   config key. It was a key, `bubblePerChapter`, and nothing read it: this
   function returns a pair by construction, so the number could be set to five
   and change nothing at all. A tunable that does not tune is worse than none,
   because it reads as the place to change this and is not. Wanting three per
   chapter means rewriting the arithmetic below, and that is the honest price.

   Scattered rather than on a stride, and derived from the chapter number so the
   answer is fixed forever without being written down anywhere. At a fixed stride
   a player has the rule after one chapter and every bubble board afterwards is
   an appointment; scattered, it stays the thing it is for, which is the run
   turning into something else without warning.

   None of this touches the graded range or the pour game's curve. ORDER is
   monotone in measured difficulty and a subsequence of a sorted sequence is
   still sorted, so it does not matter which boards are taken out: no par is
   recomputed and no ordering regenerated by moving these.
