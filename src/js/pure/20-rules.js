...
export const Rules = (() => {
  ...
  /* When a bottle counts as finished, and the one place the blast reaches into
     ...
     Spilling breaks the collapse: two units of a spilled red in two different
     bottles are each uniform and neither will ever be full, so a per-bottle test
     alone would call that board sorted while the player is plainly looking at red
     in two places. */
  ...
  /* Three stars at par, two one over, one two over. Three or more over the minimum
     ...
     An eased board caps the run at two stars. One flag rather than one per tool,
     because the reason is the same either way and a run is capped once however
     many were bought; the bubble game has called this `aided` since it shipped. Par is the minimum for the bottles
  ...
  /* Which bottles may be blasted at all.
     ...
     - Any whose removal ends the run. A blast can strand a board — take away the
       ...
       and being handed that by the rescue you just paid for is the worst version
       of this feature there is. So the question is asked in advance, on a copy,
       with the same function that ends runs.
