...
/* Measure the field of benches, choose forty of them, and write the two
   committed tables: src/measure/js/32-order.js and src/measure/js/35-pars.js.
...
const { MeasureConfig: C, MeasureLevels: L, MeasureSearch: S } =
  loadFrom('src/measure/js', ['00-config.js', '20-rules.js', '25-search.js', '30-levels.js']);
...
writeFileSync(join(root, 'src/measure/js/32-order.js'), `/* Generated by \`npm run measure:field\`. Do not edit by hand.
...
globalThis.MeasureOrder = Object.freeze({${orderEntries}});
`);

const parEntries = picked.map((f, i) => `${i + 1}:${f.par}`).join(',');
writeFileSync(join(root, 'src/measure/js/35-pars.js'), `/* Generated by \`npm run measure:field\`. Do not edit by hand.
...
globalThis.MeasurePars = Object.freeze({ last: ${picked.length}, par: Object.freeze({${parEntries}}) });
`);

console.log(`\nwrote src/measure/js/32-order.js and src/measure/js/35-pars.js for ${picked.length} levels`);
