...
/* Measure a field of cellar floors, choose sixty of them, and write the two
   committed tables: src/casks/js/pure/32-boards.js and src/casks/js/pure/35-pars.js.
   ...
   top of src/casks/js/pure/25-search.js). One multi-source sweep outward from every
   ...
   That is generation run backwards, and it replaced two earlier approaches:
     ...
     CasksSearch.component now keeps the neighbor lists it built while walking
...
/* Hand the level numbers out along the sorted pars, so the curve is
   non-decreasing BY CONSTRUCTION rather than by luck.

   ONE LEVEL PER PAR, ascending, and the spares go to the back. This used to
   divide the levels evenly across the pars and put the spares at the front,
   which gave two levels of every par and a curve of 1 1 2 2 3 3: par four
   arrived at level eight, and level eight is not where a four move board
   belongs. Four moves is the intro.

   Sliding is reversible and nothing here can be lost, so a board is only ever
   worth the thinking it takes; a shallow ramp does not protect anybody from
   anything, it just spends eight levels teaching what two teach. So the front of
   the curve starts where a board first needs a plan, and every level after it is
   a par nobody has seen. The spares go to the deep end, where an extra board of
   the same par is a genuinely different room rather than a fifth rehearsal. */
const OPENING_PAR = 4;
const ladder = pars.filter(p => p >= OPENING_PAR);

/* Levels are spread ACROSS the ladder rather than handed out one per par.

   One per par was the first try and it plateaus: the field has hundreds of
   boards at low par and a handful at high, so the ladder runs out around par
   twenty four and the top-up dumps every remaining level onto whichever par had
   boards left. That produced fourteen consecutive levels of par 24 and then a
   jump from 33 to 42, which is not a curve, it is a wall with a step in it.

   Interpolating instead means a par repeats only when there are more levels than
   rungs, and the repeats land evenly up the ladder rather than all in one place.
   A second board at the same par is a genuinely different room; fourteen of them
   is the same room fourteen times. */
const taken = new Map();
const picked = [];
for (let i = 0; i < LEVELS; i++){
  const want = ladder[Math.min(ladder.length - 1, Math.floor(i * ladder.length / LEVELS))];
  /* Walk up from the par this level wants until one has a board left. A thin
     rung borrows from the next one up rather than repeating itself. */
  for (let k = ladder.indexOf(want); k < ladder.length; k++){
    const list = byPar.get(ladder[k]);
    const used = taken.get(ladder[k]) || 0;
    if (used >= list.length) continue;
    taken.set(ladder[k], used + 1);
...
writeFileSync(join(root, 'src/casks/js/pure/32-boards.js'), `/* Generated by \`npm run casks:field\`. Do not edit by hand.
...
export const CasksBoards = Object.freeze({${boardEntries}});
`);

const parEntries = picked.map((f, i) => `${i + 1}:${f.par}`).join(',');
writeFileSync(join(root, 'src/casks/js/pure/35-pars.js'), `/* Generated by \`npm run casks:field\`. Do not edit by hand.
...
export const CasksPars = Object.freeze({ last: ${picked.length}, par: Object.freeze({${parEntries}}) });
`);

console.log(`\nwrote src/casks/js/pure/32-boards.js and src/casks/js/pure/35-pars.js for ${picked.length} levels`);
